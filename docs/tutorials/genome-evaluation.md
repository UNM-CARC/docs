---
title: "Genome assembly evaluation with QUAST and BUSCO"
description: "Evaluate genome assembly quality and completeness with QUAST and BUSCO."
type: Tutorial
tags:
  - Bioinformatics
  - Genomics
...
sources:
  - id: quickbytes
    resource: "https://github.com/UNM-CARC/QuickBytes/blob/master/genome_evaluation.md"
    title: "UNM-CARC QuickBytes: genome_evaluation.md"
    author: "team:unm-carc"
    last_modified: "2026-08-03T10:27:07-06:00"
---

# Genome assembly evaluation with QUAST and BUSCO

Whether you're using an already assembled reference genome from a database like [NCBI](https://www.ncbi.nlm.nih.gov/genome/){target=_blank} or evaluating one you've assembled for completeness, it's important to check the contiguity and completeness of your reference before using it. Here we'll discuss tools for assessing these metrics: [QUAST](http://bioinf.spbau.ru/quast){target=_blank} (Quality Assessment Tool for Genome Assemblies) for contiguity and [BUSCO](https://busco.ezlab.org/){target=_blank} (Benchmarking Universal Single-Copy Orthologs) for completeness. Here we'll go over how to run these two programs on CARC and optionally install them with conda.
...
We'll run QUAST with a submission script, here using Slurm. This is also made for Easley, being run on one core. We will store the output in a subdirectory in the submission directory. This examples runs the basic stats above on a scaffolded reference genome used [in a different QuickByte](gatk.md) borrowed from [a great paper about conservation genomics](https://academic.oup.com/gbe/article/11/7/2023/5499175){target=_blank}.
...
<p class="carc-provenance" markdown>Migrated from [UNM-CARC QuickBytes](https://github.com/UNM-CARC/QuickBytes/blob/master/genome_evaluation.md){target=_blank} (last source update 2026-08-03). Spotted a problem? [Open an issue or pull request](https://github.com/UNM-CARC/QuickBytes){target=_blank}.</p>
